...
class AppiumServer:
    def __init__(self) -> None:
        ...
        self._stdout_file_handle: Optional[Any] = None
        self._stderr_file_handle: Optional[Any] = None
    ...
    def start(self, **kwargs: Any) -> sp.Popen:
        ...
        # Handle stdout_file and stderr_file
        stdout_file = kwargs.get('stdout_file')
        stderr_file = kwargs.get('stderr_file')
        if stdout_file:
            #self._stdout_file_handle = open(stdout_file, 'a', buffering=1)  # line-buffered
            self._stdout_file_handle = open(stdout_file, 'a')
            stdout = self._stdout_file_handle
        else:
            stdout = kwargs['stdout'] if 'stdout' in kwargs else default_std

        if stderr_file:
            #self._stderr_file_handle = open(stderr_file, 'a', buffering=1)
            self._stderr_file_handle = open(stderr_file, 'a')
